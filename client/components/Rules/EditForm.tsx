...
const EditForm:React.FC = () => {
  ...
  const {data, error} = useSWR<Rule>(`/api/rules/${id}`);
  ...
  const saveRule = async () => {
    ...
    try {
      await axios.patch(`/api/rules/${id}`, {name: name, criteria: criteria, isEnabled: true});  // All new rules are enabled by default
    ...
    <div className="px-4 sm:px-6 md:px-8">
      ...
      <form className="space-y-8 divide-y divide-gray-200">
        <div className="space-y-8 divide-y divide-gray-200 sm:space-y-5">
          <div>
            <div>
              <h3 className="text-lg leading-6 font-medium text-gray-900">Edit Rule</h3>
              <p className="mt-1 max-w-2xl text-sm text-gray-500">
                Edit or delete this specific rule.
              </p>
            </div>
